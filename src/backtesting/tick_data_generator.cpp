...
private:
    ...
    std::uniform_real_distribution<> spread_dist_{0.0001, 0.0005};  // 1-5 bps
...
public:
    ...
    bool generate_csv_data(const std::string& filename, 
        ...
        for (size_t i = 0; i < num_ticks; ++i) {
            ...
            if (trade_prob_dist_(gen_) < trade_probability) {
                ...
                last_size = size_dist_(gen_) / 10; // Smaller trade sizes
            }
            ...
            // Increment timestamp by random microseconds (realistic tick frequency)
            timestamp_ns += 1000 + (gen_() % 100000); // 1μs to 100μs intervals
    ...
    bool generate_multi_symbol_data(const std::string& filename,
        ...
        for (const auto& [symbol, price] : symbols) {
            symbol_states[symbol] = {price, base_time + (gen_() % 1000000)}; // Stagger start times
        }
        ...
        for (const auto& [symbol, initial_price] : symbols) {
            ...
            for (size_t i = 0; i < ticks_per_symbol; ++i) {
                ...
                // Next tick time
                state.next_tick_time += 1000 + (gen_() % 50000); // 1μs to 50μs
            }
        }
