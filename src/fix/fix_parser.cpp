...
void FixMessage::set_field(uint32_t tag, const std::string& value) {
    ...
    // Update ordered fields for checksum calculation
...
uint8_t FixMessage::calculate_checksum() const {
    ...
    // Calculate checksum from all fields except checksum itself
...
FixParser::FixParser(size_t num_workers) 
    ...
    message_buffer_.reserve(MAX_FIX_MESSAGE_SIZE * 4); // Reserve space for multiple messages
}
...
void FixParser::feed_data(const char* data, size_t length) {
    ...
    // Extract complete messages and queue them
    std::string message;
    while (extract_next_message(message)) {
        if (running_.load()) {
            // Try to enqueue the message for parsing
    ...
    while (running_.load()) {
        if (raw_message_queue_->dequeue(raw_message)) {
            ...
            if (parse_message_internal(raw_message, parsed_message)) {
                // Successfully parsed, try to enqueue for processing
                if (!parsed_message_queue_->enqueue(std::move(parsed_message))) {
                    // Parsed message queue is full
                    if (error_callback_) {
            ...
            // No messages to parse, yield CPU
...
bool FixParser::extract_next_message(std::string& message) {
    // Find message boundary (8=FIX... followed by complete message ending with 10=XXX[SOH])
    if (message_buffer_.size() < 10) return false; // Minimum message size
    
    // Look for begin string pattern
    size_t start_pos = message_buffer_.find("8=FIX", buffer_position_);
    if (start_pos == std::string::npos) {
        // No message start found, discard processed data
        message_buffer_.clear();
        buffer_position_ = 0;
        return false;
    }
    
    // Look for body length field
    ...
    try {
        ...
        // Invalid body length, skip this data
    ...
    size_t header_end = soh_pos + 1; // Position after body length field
...
FixMessage FixMessageBuilder::build() {
    // Set mandatory header fields if not already set
...
void FixMessageBuilder::calculate_and_set_body_length() {
    // Calculate body length (everything between body length field and checksum field)
    uint32_t body_length = 0;
