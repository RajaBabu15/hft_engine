...
WebsocketClient::WebsocketClient(MatchingEngine &engine, std::string symbol)
  ...
  ws_client_.set_message_handler(std::bind(&WebsocketClient::on_message, this,
                                           std::placeholders::_1,
                                           std::placeholders::_2));
}
...
void WebsocketClient::on_message(websocketpp::connection_hdl hdl,
  ...
  for (const auto &bid : json["bids"]) {
    cmd.bids.push_back({std::stod(bid[0].get<std::string>()),
                        (Quantity)std::stod(bid[1].get<std::string>())});
  }

  for (const auto &ask : json["asks"]) {
    cmd.asks.push_back({std::stod(ask[0].get<std::string>()),
