...
namespace hft {
    ...
    public:
        void record_trade(Side side, Quantity quantity, Price price) {
            if (side == Side::BUY) {
                realized_pnl_.fetch_sub(static_cast<int64_t>(quantity) * price, std::memory_order_relaxed);
            } else {
                realized_pnl_.fetch_add(static_cast<int64_t>(quantity) * price, std::memory_order_relaxed);
            }
        }
    ...
    class RiskManager {
        ...
        public:
            ...
            void record_trade(Side side, Quantity quantity, Price price) {
                pnl_tracker_.record_trade(side, quantity, price);
            }

            int64_t get_realized_pnl() const {
                return pnl_tracker_.get_realized_pnl();
            }
