...
namespace hft {
    ...
    class OrderBook {
        private:
            ...
            std::vector<Trade> match_order_and_get_trades(OrderNode* node) {
                DEEP_PROFILE_FUNCTION();
                std::vector<Trade> trades;
                if (!node || node->hot.qty <= 0) return trades;
                ...
                if (is_buy) {
                    ...
                    if (best_bid == 0) return trades; // no bids
                    if (node->hot.price > best_bid) return trades; // cannot cross
                ...
                while (idx >= 0 && node->hot.qty > 0) {
...

                    {
                        ...
                        auto new_trades = match_simd(node, level);
            ...
            void add_limit_order(OrderNode* node) {
                DEEP_PROFILE_FUNCTION();
            ...
            std::vector<Trade> match_simd(OrderNode* node, PriceLevel& level) {
                DEEP_PROFILE_FUNCTION();
                std::vector<Trade> trades;
                if (!node || node->hot.qty <= 0) return trades;
                ...
                for (uint32_t i = 0; i < match_count && node->hot.qty > 0; ++i) {
                    ...
                    trades.emplace_back(node->hot.side == Side::BUY ? node->hot.id : book_order->hot.id,
                                        node->hot.side == Side::SELL ? node->hot.id : book_order->hot.id,
                                        node->hot.symbol, trade_qty, trade_price);
        ...
        public:
            ...
            OrderBookResult process_command(OrderNode* node, bool is_cancel) {
                DEEP_PROFILE_FUNCTION();
                OrderBookResult result;
                if (is_cancel) {
                    remove_order(node, true);
                    result.status = OrderBookResultStatus::Accepted;
                    return result;
                }
                // New order path: try to match, then add remaining
                result.trades = match_order_and_get_trades(node);
                if (node && node->hot.qty > 0) {
                    add_limit_order(node);
                    result.status = result.trades.empty() ? OrderBookResultStatus::Accepted : OrderBookResultStatus::PartiallyFilled;
                } else {
                    result.status = OrderBookResultStatus::Filled;
                }
                return result;
            }
